...
export function validateDSAItem(item: DSAItem): SchemaValidationResult {
  const errors: string[] = [];
...
export function isValidBDSAId(id: string): boolean {
...
export function validateNamingTemplate(template: string): SchemaValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];
