...
const ClinicalSchemaTab = () => {
    ...
    const loadSchema = async () => {
        try {
            ...
            const response = await fetch(`/clinical-metadata.json?t=${Date.now()}`);
            if (!response.ok) {
                throw new Error('Failed to load clinical schema');
            }
    ...
    if (loading) {
        return <div className="loading">Loading clinical schema...</div>;
    }

    if (error) {
        return (
            <div className="schema-display">
                <div className="error">Error loading clinical schema: {error}</div>
        ...
        <div className="schema-display">
            <div className="schema-section">
                <div className="schema-header-row">
                    <h3>Clinical Schema Overview</h3>
                    <button onClick={loadSchema} className="refresh-button" disabled={loading}>
                        {loading ? 'Loading...' : 'Refresh'}
                    </button>
                </div>
                <div className="schema-info">
                    <p><strong>Title:</strong> {schema?.title || 'Clinical Metadata'}</p>
...
export default ClinicalSchemaTab; 