...
const StainSchemaTab = () => {
    ...
    const loadSchema = async () => {
        try {
            ...
            const response = await fetch(`/slide-level-metadata.json?t=${Date.now()}`);
            if (!response.ok) {
                throw new Error('Failed to load stain schema');
            }
    ...
    if (loading) {
        return <div className="loading">Loading stain schema...</div>;
    }

    if (error) {
        return (
            <div className="schema-display">
                <div className="error">Error loading stain schema: {error}</div>
        ...
        <div className="schema-display">
            <div className="schema-section">
                <div className="schema-header-row">
                    <h3>Stain Schema Overview</h3>
                    <button onClick={loadSchema} className="refresh-button" disabled={loading}>
                        {loading ? 'Loading...' : 'Refresh'}
                    </button>
                </div>
                <div className="schema-info">
                    <p><strong>Title:</strong> {schema?.title || 'Slide-Level Metadata'}</p>
